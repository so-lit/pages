...
const arrayToD = points => {
  const parts = ['M', '-100,200'];
  for (const i in points) {
    parts.push(`${i * 300 / (points.length - 1) - 100},${points[i] + 25}`);
  }
  parts.push([200,200]);
  return parts.join(' ');
};
...
const sky = (hue) => h('rect', {x: -100, y: -100, height: 300, width: 300, fill: `hsl(${hue}, 35%, ${89 - nightLumOffset()*2}%)`}, []);
const sun = (hue) => {
  const sunRatio = getSunRatio();
